...
### 2025.07.08 ~ - 코드 리팩토링 및 추가 api 구현
- 배우자가 2명 이상인 경우를 고려하기 위하여 DB 테이블 정규화
- 기존 테이블에서 배우자 컬럼인 spouse_nm 컬럼을 spouse 테이블에 데이터 마이그레이션
- 각 배우자의 남편 매핑값은 기존 테이블의 남편 id값으로 외래키 지정
- 배우자 추가 및 수정 기능 구현
- 수정 화면에서 남편의 배우자를 지정 가능(관리자 권한)
- 한자, 출생년도 null 허용
